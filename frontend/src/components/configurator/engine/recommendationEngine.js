...
function createAccumulator(initialScore = 40) {
  return {
    score: initialScore,
    reasons: [],
    warnings: [],
...
function addReason(target, text) {
  if (!target.reasons.includes(text)) {
    target.reasons.push(text);
  }
}

function addWarning(target, text) {
  if (!target.warnings.includes(text)) {
    target.warnings.push(text);
  }
}

function addMissingInformation(target, text) {
  if (
...
function hasExactDimensions(piece) {
  return Boolean(
...
function getLargestDimension(piece) {
...
function buildCandidateMatches(services) {
...
function applyPieceContext(
  ...
  if (
    ...
    const smallStrength =
      profile.strengths.smallParts ?? 0;
  ...
  if (
    ...
    const largeStrength =
      profile.strengths.largeParts ?? 0;
   ...
   * DIMENSÕES EXATAS
   *
   * Ainda não usamos capacidade absoluta.
   */
...
function calculatePieceDefinition(piece) {
  const generalChecks = [
    Boolean(piece.type),
...
function buildPieceInsights(piece) {
  const insights = [];
