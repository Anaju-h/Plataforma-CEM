...
function SummaryCard({ label, value, description, href }) {
  return (
    <Link
      ...
      "
    >
      <p className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#8a959d]">
        {label}
      </p>
      ...
      <p className="mt-2 text-[12px] leading-5 text-[#7b868e]">
        {description}
      </p>
    </Link>
  );
    ...
    <div className="flex flex-col justify-between gap-4 py-4 sm:flex-row sm:items-center">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-full bg-[#eef5fb] px-2.5 py-1 text-[11px] font-semibold text-[#0057b8]">
            {status}
          </span>
        </div>

        <p className="mt-2 text-[13px] font-medium text-[#45525b]">
          {title}
        </p>

        <p className="mt-1 text-[12px] text-[#8b959c]">
          {description}
        </p>
      </div>
    </div>
...
const loadDashboard = async () => Promise.all([getCustomerRequests(), getCustomerQuotes(), getCustomerProjects()]);

export function CustomerDashboardPage() {
  const { customer } = useOutletContext();

  const state = useCustomerData(loadDashboard);
  const [requests, quotes, projects] = state.data || [[], [], []];
  ...
  if (state.error) return <CustomerApiState {...state} />;
  return (
    <div className="mx-auto max-w-[1180px] px-5 py-8 sm:px-7 lg:px-8 lg:py-10">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 max-w-[590px] text-[14px] leading-6 text-[#6e7981]">
      ...
      {loading ? (
        <div className="mt-8 rounded-[14px] border border-[#dfe6ea] bg-white px-6 py-12 text-center text-[14px] text-[#7b868e]">
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[1.55fr_0.8fr]">
            <section className="rounded-[14px] border border-[#dfe6ea] bg-white p-6">
              ...
              <div className="mt-4 divide-y divide-[#edf1f3]">
                {!recentActivity.length && <p>Nenhum atendimento por enquanto. Crie sua primeira solicitação.</p>}
            ...
            <aside className="rounded-[14px] border border-[#dfe6ea] bg-white p-6">
              <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-[#8b969e]">
                Sua empresa
              </p>
              ...
              <div className="mt-5 space-y-4 border-t border-[#edf1f3] pt-5">
                <div>
                  <p className="text-[11px] uppercase tracking-[0.08em] text-[#9aa3aa]">
                    Usuário
                  </p>

                  <p className="mt-1 text-[13px] font-medium text-[#45525b]">
                    {customer.user.name}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="mt-1 break-all text-[13px] font-medium text-[#45525b]">
                    {customer.user.email}
                  </p>
                </div>
