...
export function TeamPage() {
  ...
  if (loadError || !overview) return <ApiState eyebrow="Gestão" title="Equipe" description="Perfis ativos, responsabilidades e acesso à operação do laboratório." loading="Carregando equipe..." error={loadError} onRetry={() => { setLoadError(""); setRetry(value => value + 1); }} />;
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    </div><p className="internal-help-text mt-3 text-[#607989]">Contagens dos registros.</p>
    <div className="mt-5 grid gap-5">
      {activeMembers.map(member => <section key={member.id} className="rounded-[22px] border border-[#cddbe3] bg-white/85 p-5 sm:p-6">
        ...
        <p className="internal-help-text mt-3 text-[#607989]">Carga atribuída: {member.workload.total} registro(s).</p>
