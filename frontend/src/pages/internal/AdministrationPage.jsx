...
import { getAttentionRules, getAuditEvents, getGeneralSettings, loadAuditEvents, updateAttentionRule, updateGeneralSettings, registerAuditEvent } from "../../services/administrationService";
import { useCurrentUser } from "../../hooks/useCurrentUser";
import { DemoDataPanel } from "../../components/internal/DemoDataPanel";

const tabs = [{ id: "general", label: "GERAL" }, { id: "attention", label: "REGRAS DE ATENÇÃO" }, { id: "demo", label: "DADOS DE DEMONSTRAÇÃO" }, { id: "audit", label: "AUDITORIA" }];
...
export function AdministrationPage() {
  ...
  const refreshAudit = () => { setEvents(getAuditEvents()); loadAuditEvents().then(setEvents); };
  ...
  return <div className="mx-auto max-w-[1500px]">
    ...
    <p className="internal-help-text mt-4 text-[#526d7c]">As alterações são gravadas no banco e registradas na auditoria.</p>
    <div className="mt-5 grid gap-4 sm:grid-cols-3">{[["Perfil ativo", user.accessProfile], ["Perfis e acessos", "Gestão → Equipe"], ["Regras habilitadas", rules.filter(rule => rule.enabled).length]].map(([label, value]) => <div key={label} className="rounded-[18px] border border-[#cadce6] bg-white/65 p-5"><p className="internal-field-label text-[#607989]">{label}</p><p className="mt-2 text-xl font-semibold text-[#17394f]">{value}</p></div>)}</div>
    ...
    <div className="mt-5">
      ...
      {activeTab === "attention" && <><p className="internal-body mb-4 text-[#526d7c]">Limites utilizados em Meu trabalho para destacar inatividade e proximidade de prazo. Alterações são aplicadas ao editar os campos.</p><div className="flex flex-col gap-5">{rules.map(rule => <section className={panelClass + " border-l-4 " + (rule.enabled ? "border-l-[#096ab2]" : "border-l-[#9aaeb9]")} key={rule.id}>
        <div className="flex items-start justify-between gap-4"><div><p className="internal-field-label text-[#607989]">{rule.entity}</p><h2 className="mt-1 text-xl font-semibold text-[#17394f]">{rule.situation}</h2></div>
          <button type="button" role="switch" aria-checked={rule.enabled} aria-label={`Ativa: ${rule.entity} · ${rule.situation}`} onClick={() => changeRule(rule.id, { enabled: !rule.enabled })} className="flex shrink-0 items-center gap-2.5 rounded-full text-[15px] font-medium text-[#31566d] outline-none focus-visible:ring-2 focus-visible:ring-[#096ab2] focus-visible:ring-offset-4">
