...
function ExplorationStage({
  ...
  /*
   * Cria uma assinatura estável da lista.
   *
   * Exemplo:
   *
   * prismo|duramax|o-inspect
   *
   * Assim conseguimos detectar quando o usuário altera
   * suas necessidades e o conjunto de máquinas muda.
   */
  const machineKey =
    machines.join("|");

  /*
   * Sempre que a seleção da Etapa 01 muda,
   * reiniciamos o carrossel.
   *
   * Isso evita manter um índice pertencente
   * à lista anterior.
   */
  useEffect(() => {
    setActiveIndex(0);
   ...
   * PROTEÇÃO PRINCIPAL
   *
   * Mesmo antes do useEffect conseguir reiniciar o estado,
   * nunca permitimos usar um índice que não exista.
   */
  const safeIndex =
    activeIndex >= 0 &&
    activeIndex <
  ...
  /*
   * Como já verificamos machines.length > 0,
   * deve existir uma máquina.
   *
   * Mesmo assim mantemos proteção extra.
   */
  const activeMachine =
    activeMachineId
      ? getMachineProfile(
...
function EvaluationStage({
  ...
  /*
   * Sempre garante que a máquina selecionada
   * ainda exista no ranking atual.
   */
  useEffect(() => {
    if (
      matches.length ===
  ...
  /*
   * Proteção adicional caso algum ID inválido
   * chegue ao recommendation engine.
   */
  if (!machine) {
    return (
      <StageShell>
