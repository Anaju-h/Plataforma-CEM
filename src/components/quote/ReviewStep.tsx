    ...
    <div>
      ...
      >
        ...
        <div className="mt-6">
          <ReviewItem
            label="Observações"
            value={project.observations || "Nenhuma observação adicionada"}
          />
        </div>
              ...
              value={
                piece.locationCity || piece.locationState
                  ? `${piece.locationCity || "Cidade não informada"}${
                      piece.locationState ? ` - ${piece.locationState}` : ""
                    }`
                  : "Não informado"
              }
            />
...
function getDeadlineLabel(data: ProjectData) {
...
function getTransportLabel(piece: PieceData) {
...
function getMovableLabel(
  movable: PieceData["movable"],
) {
  const labels = {
    yes: "Sim",
    no: "Não",
