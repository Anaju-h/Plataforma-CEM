...
public class RequestService {
  ...
  @Transactional public Response create(Create dto) {
    if(Set.of("cliente","público","publico","configurador").contains(clean(dto.origin(),"").toLowerCase(Locale.ROOT))) throw bad("Use a área do cliente ou o formulário público para registrar esta origem.");
    return create(dto,null);
  }
  @Transactional public Response createForCustomer(Create dto,CustomerUserEntity user) {
    var contact=new Contact(user.company.name,user.name,user.email,user.phone);
    boolean configurator="Configurador".equalsIgnoreCase(clean(dto.origin(),""));
    return create(new Create(contact,dto.project(),null,dto.pieces(),configurator?"Configurador":"Cliente",configurator?"Configurador on-line":"Área do cliente",false,configurator?dto.configuration():null),user);
  }
  /** Public site form: origin is fixed on the server and a one-time claim token lets the requester link the SOL to a new account. */
  @Transactional public Map<String,Object> createPublic(Create dto) { return createPublic(dto,"Público","Formulário público",null); }
  /** Configurador on-line: mesma regra do formulário público, com o snapshot técnico da configuração. */
  @Transactional public Map<String,Object> createFromConfigurator(Create dto) { return createPublic(dto,"Configurador","Configurador on-line",dto.configuration()); }
  private Map<String,Object> createPublic(Create dto,String origin,String channel,Map<String,Object> configuration) {
    String token=br.org.senai.lab.security.Tokens.random();
    Response created=create(new Create(dto.contact(),dto.project(),null,dto.pieces(),origin,channel,false,configuration),null,br.org.senai.lab.security.Tokens.sha256(token));
    Map<String,Object> result=new LinkedHashMap<>();
    result.put("id",created.id());result.put("claimToken",token);result.put("company",created.company());result.put("contact",created.contact());result.put("email",created.email());result.put("phone",created.phone());
    return result;
  }
  private Response create(Create dto,CustomerUserEntity user) { return create(dto,user,null); }
  private Response create(Create dto,CustomerUserEntity user,String claimHash) {
    ...
    if(user!=null){r.customerCompanyId=user.company.id;r.customerUserId=user.id;}
    r.claimTokenHash=claimHash;
    r.id=UUID.randomUUID(); r.requestCode=formatRequestCode(repository.nextNumber());
    r.source=Boolean.TRUE.equals(dto.demo())&&user==null&&claimHash==null?"demo":"real"; r.origin=clean(dto.origin(),"Interno"); r.channel=clean(dto.channel(),dto.internal()==null?"Registro interno":clean(dto.internal().channel(),"Registro interno"));
    ...
    // Configurador: a tecnologia mais aderente já chega como sugestão para a análise técnica (e segue para o ORC).
  ...
  private void event(RequestEntity r,String action,String description){eventAs(r,action,description,actor());}
  private void eventAs(RequestEntity r,String action,String description,String who){HistoryEntity h=new HistoryEntity();h.id=UUID.randomUUID();h.request=r;h.occurredAt=now();h.action=action;h.actor=who;h.description=description;r.history.add(h);}
